// services/sheets.js — Google Sheets comme base de données users
...
const COL = {
  ...
  avatar_url:      5,  // F
  drive_folder_id: 6,  // G
  api_keys_enc:    7,  // H (JSON chiffré)
  preferences_enc: 8,  // I (JSON chiffré: thème, modèles par défaut...)
  created_at:      9,  // J
  last_login:      10, // K
};
const NUM_COLS = 11;
...
// ── Initialise le sheet (crée l'en-tête si absent) ──────────────
async function initSheet() {
  ...
  try {
    ...
    if (!res.data.values || res.data.values[0][0] !== 'id') {
      ...
      console.log('✅ Sheet Users initialisé');
    }
  } catch (e) {
    // La feuille n'existe peut-être pas — on la crée
...
// ── Trouver le numéro de ligne d'un user (1-indexed, incl. header) ─
async function findRowIndex(id) {
  const rows = await getAllRows();
  const idx  = rows.findIndex(r => r[COL.id] === id);
  return idx === -1 ? -1 : idx + 2; // +2 : header + 1-index
}
...
// ── Mettre à jour des champs d'un user ───────────────────────────
